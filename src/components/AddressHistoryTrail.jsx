...
function HistoryRow({ record, index }) {
  ...
  return (
    <div className="relative">
      {/* Timeline line */}
      {index > 0 && (
        <div className="absolute left-[18px] -top-3 w-px h-3 bg-slate-700/60" />
      )}
      ...
      <div className="rounded-xl border border-slate-700/40 bg-slate-900/40 overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className={`text-xs px-2 py-0.5 rounded-full border ${reasonClass} font-medium`}>
              {record.reason}
            </span>
...
export default function AddressHistoryTrail({ userEmail, currentScore }) {
  ...
  useEffect(() => {
    (async () => {
      const records = await base44.entities.AddressHistory.filter(
        { user_email: userEmail }, '-deprecated_at', 20
      );
      setHistory(records);
      setLoading(false);
    })();
    ...
    <div>
      ...
      <div className="flex items-start gap-2.5 p-3 rounded-xl bg-amber-500/8 border border-amber-500/15 mb-4">
        ...
        <p className="text-xs text-slate-500 leading-relaxed">
          <span className="text-amber-400/80 font-medium">Why does my score reset?</span> Each address builds its own trust score from scratch — this prevents score "portability" fraud, where someone could claim trust earned at a different location. Your previous scores are permanently on record for banks to audit.
        </p>
      </div>
