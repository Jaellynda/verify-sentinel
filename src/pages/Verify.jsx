  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className={`relative w-20 h-20 rounded-2xl flex items-center justify-center border-2 transition-all ${
        isVerified ? 'bg-emerald-500/20 border-emerald-500/60 shadow-[0_0_40px_rgba(16,185,129,0.4)]'
        : isPartial ? 'bg-blue-500/20 border-blue-500/40 shadow-[0_0_20px_rgba(59,130,246,0.3)]'
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 py-12">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-green-500/30 bg-green-500/10 text-green-400 text-xs font-medium uppercase tracking-widest mb-4">
            <Zap className="w-3 h-3" /> Business & Delivery
          </div>
        ...
        <div className="relative mb-8">
          ...
          <div className="flex gap-3 p-4 rounded-2xl border border-zinc-800" style={{ background: '#18181b' }}>
        ...
        {result && (
          <div className="space-y-5">
            <div className="p-8 rounded-3xl border text-center" style={{
              background: '#18181b',
              ...
              <div className="mt-5 mb-3">
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold ${
                  result.status === 'Verified' ? 'bg-emerald-500/20 border border-emerald-500/40 text-emerald-400'
                  : result.status === 'Partial' ? 'bg-green-500/20 border border-green-500/40 text-green-400'
              ...
              <p className="text-xl font-bold text-white tracking-widest mb-1 cursor-pointer transition-all"
                style={{ fontFamily: '"JetBrains Mono", monospace' }}
                onMouseEnter={() => setHovering(true)}
                onMouseLeave={() => setHovering(false)}>
                {hovering
                  ? Array.from(result.sentinel_id).map((c) => c === '-' ? '-' : String.fromCharCode(48 + Math.floor(Math.random() * 22))).join('')
                  : result.sentinel_id}
              </p>
            ...
            {landmarks.length > 0 && (
              <div className="p-6 rounded-3xl border border-zinc-800" style={{ background: '#18181b' }}>
        ...
        {!result && !notFound && !loading && (
          <div className="p-8 rounded-3xl border border-zinc-800 text-center" style={{ background: '#18181b' }}>
