...
import { Shield, Copy, CheckCircle, AlertCircle, Navigation, MapPin, Target, Crosshair, Download } from 'lucide-react';
...
function HexPolygon({ h3Index, phase }) {
  ...
  return (
    <Polygon positions={positions} pathOptions={{
      color: style.color, fillColor: style.fillColor,
      fillOpacity: style.fillOpacity, weight: style.weight, dashArray: style.dashArray,
    }} />
  );
}
...
export default function GetMyID({ lang = 'en' }) {
  ...
  const canSave = (phase === 'locked' && accuracy !== null && accuracy <= 10) || phase === 'manual';
  ...
  const acquireGPS = useCallback(() => {
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        setCurrentRes(res);
        setPhase(p);
        if (mapRef.current) mapRef.current.setView([latitude, longitude], res === 10 ? 18 : res === 8 ? 16 : 14);
        if (p === 'locked') {
    ...
    try {
      ...
      if (landmarks.length > 0) {
        const landmarkRows = landmarks.map(lm => ({
          sentinel_address_id: addressRecord.id,
          h3_index: sentinelData.h3_index,
          landmark_type: lm.landmark_type || lm.type,
          direction: lm.direction,
          distance_meters: lm.distance_meters || (lm.distance ? parseFloat(lm.distance) : null),
          description_text: lm.description_text || lm.description,
  ...
  const handleDownloadPDF = async () => {
    ...
    doc.text('SENTINEL ADDRESS CERTIFICATE', W / 2, 22, { align: 'center' });
    doc.setFontSize(10);
    doc.setTextColor(100, 116, 139);
    doc.text('VerifySentinel — Spatial Identity Platform for East Africa', W / 2, 30, { align: 'center' });
    doc.setDrawColor(74, 222, 128);
    doc.setLineWidth(0.4);
    doc.line(20, 35, W - 20, 35);
    doc.setFillColor(20, 20, 20);
    doc.roundedRect(20, 42, W - 40, 28, 3, 3, 'F');
    doc.setTextColor(100, 116, 139);
    doc.setFontSize(8);
    doc.text('SENTINEL ID', W / 2, 51, { align: 'center' });
    doc.setTextColor(74, 222, 128);
    doc.setFontSize(18);
    doc.setFont('courier', 'bold');
    doc.text(savedAddress.sentinel_id, W / 2, 63, { align: 'center' });
    if (qrImg) {
      const qrSrc = qrImg.tagName === 'IMG' ? qrImg.src : qrImg.toDataURL();
      doc.addImage(qrSrc, 'PNG', W / 2 - 27, 78, 54, 54);
    }
    doc.setTextColor(100, 116, 139);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('Scan to verify', W / 2, 138, { align: 'center' });
    const fields = [
      ['Country', savedAddress.country || '—'],
      ['Status', savedAddress.status || 'Visitor'],
      ['Residency Type', savedAddress.residency_type || '—'],
      ['Trust Score', `${savedAddress.trust_score || 30}/100`],
      ['Hex Index', savedAddress.h3_index],
      ['Coordinates', `${savedAddress.hex_center_lat?.toFixed(5)}, ${savedAddress.hex_center_lng?.toFixed(5)}`],
    ...
    doc.setFontSize(7);
    doc.text('This document is generated by VerifySentinel. Verify at verifysentinel.com/verify', W / 2, H - 12, { align: 'center' });
    doc.save(`Sentinel-${savedAddress.sentinel_id}.pdf`);
  };
  ...
  return (
    <div className="min-h-screen pt-16" style={{ background: '#0a0a0a' }}>
      ...
      <div className="relative z-10 max-w-lg mx-auto px-4 py-10">
            ...
            <div className="p-6">
              ...
              {phase === 'idle' || phase === 'error' ? (
                ...
                <div className="space-y-3">
                  <button onClick={() => setStep(1)} disabled={!canSave}
        ...
        {/* STEP 1: Landmarks */}
        {step === 1 && (
          <div className="p-8 rounded-3xl border border-zinc-800" style={{ background: '#18181b' }}>
        ...
        {step === 2 && savedAddress && (
          <div className="space-y-5">
            <div className="p-8 rounded-3xl border border-green-500/30 text-center" style={{ background: '#18181b' }}>
              ...
              <button onClick={handleDownloadPDF}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-blue-500/30 bg-blue-500/10 text-blue-400 text-sm font-medium hover:bg-blue-500/20 transition-all mb-4">
